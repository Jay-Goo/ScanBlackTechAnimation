...
public class BrokenAnimator extends ValueAnimator{
    ...
    public BrokenAnimator(BrokenView brokenView,Rect rect,Bitmap bitmap,Point point,BrokenConfig config){
        ...
        offsetX = mTouchPoint.x - rect.left;
        offsetY = mTouchPoint.y - rect.top;
        // Make the touchPoint be the origin of coordinates
        rect.offset(-mTouchPoint.x, -mTouchPoint.y);
     ...
     * @param bitmap
     */
    private void buildPieces(Bitmap bitmap){
    ...
    private void buildPaintShader(Bitmap bitmap){
    ...
    public boolean draw(Canvas canvas) {
        ...
        if(getStage() == STAGE_BREAKING) {
            ...
            if(piecesNum == 0) {
                ...
                mBrokenView.onBrokenFallingEnd();
            }
        }
...
//        release();
    ...
    private void release(){
        ...
        mBrokenView.clearAnimation();
        mBrokenView = null;

        for(Piece p : pieces){
            RecycleUtils.recycleBitmap(p.bitmap);
        }
