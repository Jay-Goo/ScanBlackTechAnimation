...
public class DepthRelativeLayoutContainer extends RelativeLayout {
    ...
    void setup() {
        ...
        softShadow = (NinePatchDrawable) ResourcesCompat.getDrawable(getResources(), R.drawable.shadow, null);
        roundSoftShadow = ResourcesCompat.getDrawable(getResources(), R.drawable.round_soft_shadow, null);
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        if (child instanceof DepthRelativeLayout && !isInEditMode()) {
            ...
            if (dl.isCircle()) {
                ...
                if (dl.getCustomShadowElevation() > 0) {
//                    dl.getCustomShadow().drawShadow(canvas, dl, softShadow);
    ...
    private void drawCornerBaseShape(DepthRelativeLayout dl, Canvas canvas, float[] src) {
        ...
        edgePath.reset();
        edgePath.addRoundRect(new RectF(0, 0, dl.getWidth(), dl.getHeight()) , dl.getWidth() / 2f, dl.getHeight() / 2f, Path.Direction.CCW);
